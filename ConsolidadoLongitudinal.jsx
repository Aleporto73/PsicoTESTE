...
export default function ConsolidadoLongitudinal({ sessions, childName, data, onBack }) {
    ...
    const longitudinalData = useMemo(() => {
        return childSessions.map((session, index) => {
            ...
            const totalBlocks = data.total_blocks;
            const values = Object.values(scores);

            const dominados = values.filter(v => v === 'dominado').length;
            const emergentes = values.filter(v => v === 'emergente').length;
            const scoreTotal = values.reduce((acc, v) => acc + mapScoreValue(v), 0);
            const percentTotal = ((scoreTotal / totalBlocks) * 100).toFixed(1);

            // Dados por Domínio
            const byDomain = data.domains.map(domain => {
                const dScores = domain.blocks.map(b => scores[b.block_id]).filter(Boolean);
                const dTotal = dScores.reduce((acc, v) => acc + mapScoreValue(v), 0);
                return {
                    id: domain.domain_id,
                    name: domain.domain_name,
                    percent: ((dTotal / domain.total_blocks) * 100).toFixed(1)
                };
            });
...
function getStyles() {
    return `
        ...
        .bar-value { position: absolute; top: -30px; left: 50%; transform: translateX(-50%); font-weight: 800; font-size: 0.9rem; color: #1e293b; width: 60px; text-align: center; }
        .bar-date { color: #94a3b8; font-size: 0.85rem; font-weight: 500; }
        ...
        .mini-bar-label { position: absolute; left: 0.5rem; font-size: 0.7rem; font-weight: 800; color: #1e293b; background: rgba(255,255,255,0.6); padding: 0 4px; border-radius: 4px; }
        ...
        .v-tag { background: #3b82f6; color: white; width: fit-content; padding: 0.25rem 0.75rem; border-radius: 6px; font-weight: 800; font-size: 0.85rem; }
        .p-val { font-weight: 700; color: #0f172a; }
        .t-status { font-weight: 700; font-size: 0.95rem; }
        ...
        .domain-separator td { font-weight: 800; color: #1d4ed8; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.5px; }
        .m-id { font-family: monospace; font-weight: 600; color: #64748b; font-size: 0.85rem; }
